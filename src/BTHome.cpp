...
void BTHome::begin(String dname, bool encryption, String key, bool trigger_based_device, bool add_mac, bool add_packetId) {
    /*
    ...
    */
    ...
    begin(dname, encryption, bind_key, trigger_based_device, add_mac, add_packetId);
}

void BTHome::begin(String dname, bool encryption, uint8_t const* const key, bool trigger_based_device, bool add_mac, bool add_packetId) {
    /*
    ...
    */
    ...
    this->m_addMac = add_mac;
    this->m_addPacketId = add_packetId;
    resetMeasurement();
}
...
void BTHome::buildPacket() {
    ...
    */
    ...
    if (this->m_addMac)
        adv_info |= ADD_MAC_ADDR;
    if (this->m_triggerdevice)
        adv_info |= ADD_TRIGGER;
    if (this->m_encryptEnable)
        adv_info |= ADD_ENCRYPT;

    serviceData += adv_info; // Add the BTHome version and features to the Service Data

    if (this->m_addMac) {
        const ble_addr_t *addrs = BLEDevice::getAddress().getBase();
        serviceData += addrs->val[5]; // Add the MAC address to the Service Data
        serviceData += addrs->val[4];
        serviceData += addrs->val[3];
        serviceData += addrs->val[2];
